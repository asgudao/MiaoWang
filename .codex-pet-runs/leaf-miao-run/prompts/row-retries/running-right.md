Create Codex pet row `running-right` for `leaf-miao`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: 2D cartoon sticker-style cat, orange-cream fur, white belly and paws, subtle maple-leaf accent on forehead or tail tip, compact mascot, readable at pet size, warm and practical. Preserve silhouette, face, palette, material, proportions, markings, and props.
